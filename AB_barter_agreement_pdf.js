...
const printPdf = async () => {
    ...
    formElements.forEach((element) => {
      if (element.Type === 'text') {
        doc.text(
          ...
          10,
          doc.internal.getNumberOfPages() * 40 + 10
        );
      }
      if (element.Type === 'textarea') {
        ...
        for (const line of element.value.split('\n')) {
          doc.text(line, 10, pos);
          pos += lineHeight;
        }
      }
    });
  ...
  // Set the event listener to the 'savePdf' button
